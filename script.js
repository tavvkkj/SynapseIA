document.addEventListener('DOMContentLoaded', () => {
    ...
    const handleChatSubmit = async (event, promptOverride = null) => {
        ...
        if (selectedFiles.length > 0) {
             selectedFiles.forEach(file => {
                 ...
                 if (file.type.startsWith('image/')) {
                     filePart = { inline_data: { mime_type: file.type, data: file.base64 } };
                 } else {
                     filePart = { text: `O usuário anexou o arquivo '${file.name}' com o seguinte conteúdo:\n\n\`\`\`\n${file.content}\n\`\`\`` };
                 }
                 userMessage.parts.push(filePart);
                 userMessage.filesInfo.push({ name: file.name, type: file.type, content: file.content });
             });
        }
        ...
        appendMessage(promptText, 'user', userMessage.filesInfo);
        chatInput.value = '';
        autoResizeTextarea();
        resetFileSelection();
    ...
    const saveCurrentChat = () => {
        ...
        if (firstUserMessage) {
            ...
            } else if (firstUserMessage.filesInfo && firstUserMessage.filesInfo.length > 0) {
                title = `Arquivos: ${firstUserMessage.filesInfo.map(f => f.name).join(', ')}`;
            }
        }
    ...
    const loadChat = (chatId) => {
        ...
        currentChatHistory.forEach(message => {
            ...
            const filesInfo = message.role === 'user' ? (message.filesInfo || []) : [];
            appendMessage(text, type, filesInfo);
        });
        renderHistorySidebar();
    };
    ...
    const appendMessage = (text, type, filesInfo = []) => {
        if (welcomeContainer) welcomeContainer.style.display = 'none';
    ...
    const handleFileSelection = (files) => {
        ...
        for (const file of files) {
            ...
            if(file.type.startsWith('image/')) {
                ...
                const newFile = { name: file.name, type: file.type, size: file.size, content: `[Arquivo binário, conteúdo não exibido]` };
                selectedFiles.push(newFile);
                displayFilePreview();
                updateSendButtonState();
            }
    ...
    const displayFilePreview = () => {
        const container = document.getElementById('file-preview-container-items');
        container.innerHTML = '';
        if (selectedFiles.length === 0) {
        ...
        selectedFiles.forEach((file, index) => {
            let previewHTML = '';
            if (file.type.startsWith('image/')) {
                previewHTML = `<img src="${file.content}" alt="Preview">`;
            } else {
                const iconClass = getIconForFileType(file.name);
            ...
            item.innerHTML = `
                ...
                <button class="remove-file-btn" data-index="${index}" title="Remover arquivo">&times;</button>
            `;
    ...
    const updateSendButtonState = () => {
        ...
        chatSendBtn.disabled = !hasText && !hasFiles;
    };
